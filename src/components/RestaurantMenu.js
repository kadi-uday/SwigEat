...
const categories = resInfo?.data?.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards.filter((c) => c.card?.card?.["@type"] === "type.googleapis.com/swiggy.presentation.food.v2.ItemCategory");
    ...
    return (
        <div className="menu m-25 mx-20  p-5 px-20 font-['Segoe UI']">
            <h2 className="rest-name max-w-8/12 mx-auto text-3xl font-bold mt-[15px] mb-2.5 pr-20 text-center">{name}</h2>
            <div className="res-details bg-white p-5 my-5 mx-auto max-w-9/12 w-full rounded-[20px] shadow-xl border-[3px] border-[#eee] ">
